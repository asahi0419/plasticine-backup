...
const migrate = (knex) => async (m) => {
  ...
  console.log('=============================================');
  console.log('\x1b[37m%s\x1b[0m', `Multiselect Array (string) values sorting`);
  console.log('\x1b[37m%s\x1b[0m', `Models: ${models.length}`);
  console.log('=============================================');

  await Promise.each(uniqBy(models, 'id'), async (model) => {
    ...
    await Promise.each(modelFields, async (field) => {
      ...
      await Promise.each(records, async (record) => {
        ...
        const value = record[field.alias].split(',').sort().join(',');
