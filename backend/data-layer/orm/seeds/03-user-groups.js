export default {
  ...
  fields: [
    ...
    },
    {
      ...
      readonly_when_script: `['__core', '__public'].includes(p.record.getValue('alias'))`,
      __lock: ['delete'],
    },
  ],
  ui_rules: [
...
p.record.getField('name').onChange((oldValue, newValue) => {
  ...
  records: [
    { name: 'System Administrators', alias: '__core', __lock: ['delete'] },
    { name: 'Public', alias: '__public', __lock: ['delete'] },
  ],
  views: [
    {
      name: 'Default',
  ...
  layouts: [
    {
      ...
      type: 'grid',
      options: {
        columns: ['id', 'name', 'created_at', 'updated_at', 'created_by', 'updated_by'],
        columns_options: {},
        sort_order: [
