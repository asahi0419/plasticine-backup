...
beforeAll(async () => {
  ...
  t.fields = {
    self: {
      ...
      datetime: await manager('field').create({ model: t.models.self.id, type: 'datetime', options: JSON.stringify({ format: DATE_FORMAT }) }),
    },
    foreign: {
      datetime: await manager('field').create({ model: t.models.foreign.id, type: 'datetime', options: JSON.stringify({ format: DATE_FORMAT }) }),
    },
  };
